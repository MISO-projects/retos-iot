...
// Duración del sonido del buzzer en segundos
#define BUZZER_DURATION 3
// Pin del motor (ventilador) controlado por L293D
#define MOTOR_PIN D1
...
#define USER "ja.penat1" // TODO Reemplace UsuarioMQTT por un usuario (no administrador) que haya creado en la configuración del bróker de MQTT.
const char MQTT_HOST[] = "54.209.126.215"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
const int MQTT_PORT = 8082;
const char MQTT_USER[] = USER;
//Contraseña de MQTT
const char MQTT_PASS[] = "Test123!"; // TODO Reemplace ContrasenaMQTT por la contraseña correpondiente al usuario especificado.
...
float humi;
// Indica si el buzzer debe sonar
bool buzzerActive = false;
// Tiempo en que se activó el buzzer
long long int buzzerTime = 0;
// Indica si el ventilador está encendido
 ...
 * Además muestra el estado del ventilador.
 */
void displayMeasures() {
  ...
  display.println(fanActive ? "ON" : "OFF");
}

/**
 * Muestra una alerta formateada en la pantalla.
 * Parsea el mensaje con formato "ALERTA tipo valor limite"
 * y lo muestra en un recuadro resaltado.
 */
void displayAlert(String alertMsg) {
  // Parsear: "ALERTA temperatura 10 30"
  int firstSpace = alertMsg.indexOf(' ');
  int secondSpace = alertMsg.indexOf(' ', firstSpace + 1);
  int thirdSpace = alertMsg.indexOf(' ', secondSpace + 1);

  if (firstSpace < 0 || secondSpace < 0 || thirdSpace < 0) {
    // Formato no reconocido, mostrar el mensaje tal cual
  ...
  String type = alertMsg.substring(firstSpace + 1, secondSpace);
  String minVal = alertMsg.substring(secondSpace + 1, thirdSpace);
  String maxVal = alertMsg.substring(thirdSpace + 1);
  ...
  // Recuadro de alerta
  display.drawRect(0, 26, 128, 38, SSD1306_WHITE);

  // Encabezado "!! ALERTA !!" con fondo blanco y texto negro
  display.fillRect(1, 27, 126, 11, SSD1306_WHITE);
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (data.indexOf("FAN ON") >= 0) {
    // Comando de ventilador: activar motor
    fanActive = true;
    digitalWrite(MOTOR_PIN, HIGH);
    Serial.println(">> Ventilador ENCENDIDO");
  } else if (data.indexOf("FAN OFF") >= 0) {
    // Comando de ventilador: apagar motor
    fanActive = false;
    digitalWrite(MOTOR_PIN, LOW);
    Serial.println(">> Ventilador APAGADO");
  } else if (data.indexOf("ALERT") >= 0) {
    alert = data;
    // Activar el buzzer al recibir una alerta
    buzzerActive = true;
    buzzerTime = millis();
...
/**
 * Verifica si el buzzer debe apagarse después de la duración configurada.
 */
void checkBuzzer() {
  if (buzzerActive && (millis() - buzzerTime) >= BUZZER_DURATION * 1000) {
...
void setup() {
  ...
  // Configurar el pin del motor (ventilador) como salida
  pinMode(MOTOR_PIN, OUTPUT);
  digitalWrite(MOTOR_PIN, LOW); // Ventilador apagado al inicio

  // Inicializar I2C con los pines correctos de la pantalla OLED
  // SDA = GPIO14 (D6), SCL = GPIO12 (D5)
  Wire.begin(14, 12);
...
void loop() {
  ...
  // Apagar el buzzer si ya pasó la duración configurada
  checkBuzzer();
