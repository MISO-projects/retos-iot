...
class AnomalyDetectionView(View):
    ...
    Patron Esquema Estrella (Postgre): cada registro en Data tiene un solo
    value y un time (DateTimeField). Se usa Django ORM con Avg y StdDev
    para calcular estadisticas por estacion y detectar anomalias.
    """
    ...
    def get(self, request, measurement_name):
        ...
        filters = {'measurement': measurement, 'time__gte': start, 'time__lte': end}
        if city_name:
            filters['station__location__city__name'] = city_name

        base_qs = Data.objects.filter(**filters)
        total_readings = base_qs.count()

        sensor_stats = (
            base_qs.values('station')
            .annotate(avg_val=Avg('value'), stddev_val=StdDev('value'))
        )
        stats_map = {
            s['station']: (s['avg_val'], s['stddev_val'])
            for s in sensor_stats
            if s['avg_val'] is not None and s['stddev_val'] is not None and s['stddev_val'] > 0
        }

        anomalies = []
        readings = (
            base_qs
            .select_related('station', 'station__user', 'station__location',
                            'station__location__city', 'station__location__state',
                            'station__location__country', 'measurement')
            .filter(station__id__in=stats_map.keys())
            .order_by('-time')
        )
        for reading in readings:
            avg_val, stddev_val = stats_map[reading.station_id]
            deviation = abs(reading.value - avg_val)
            if deviation > threshold * stddev_val:
                loc = reading.station.location
                anomalies.append({
                    "time": reading.time.isoformat(),
                    "value": reading.value,
                    "measurement": measurement.name,
                    "unit": measurement.unit,
                    "user": reading.station.user.login,
                    "location": f"{loc.city.name}, {loc.state.name}, {loc.country.name}",
                    "station_avg": round(avg_val, 2),
                    "station_stddev": round(stddev_val, 2),
                    "deviation_factor": round(deviation / stddev_val, 2),
                })
