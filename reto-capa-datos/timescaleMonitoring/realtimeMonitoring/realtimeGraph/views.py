...
class AnomalyDetectionView(View):
    """
    GET /api/anomalies/<measurement_name>/
    Patron Blob (Timescale): cada registro en Data tiene listas `values` y
    `times`, ademas de `avg_value`, `min_value`, `max_value` precalculados.
    Se usa SQL raw con time_bucket() y unnest() para expandir los arrays
    y detectar anomalias por ventana temporal.
    """
    ...
    def get(self, request, measurement_name):
        try:
            ...
            return JsonResponse(
                {"error": f"Measurement '{measurement_name}' not found"}, status=404
            )
        ...
        if start is None and end is None:
            start = datetime.now() - dateutil.relativedelta.relativedelta(weeks=4)
        ...
        where_clauses = ['d."time" >= %s', 'd."time" <= %s', "m.name = %s"]
        params = [start_ts, end_ts, measurement_name]

        if city_name:
            where_clauses.append("ci.name = %s")
        ...
        query = f"""
            WITH expanded AS (
                SELECT
                    d."time" AS data_time, d.base_time,
                    unnest(d."values") AS individual_value,
                    unnest(d.times) AS individual_time_offset,
                    u.login AS user_login,
                    ci.name AS city_name, st.name AS state_name, co.name AS country_name,
                    m.name AS measurement_name, m.unit AS measurement_unit,
                    s.id AS station_id
                FROM "realtimeGraph_data" d
                JOIN "realtimeGraph_station" s ON d.station_id = s.id
                JOIN "realtimeGraph_user" u ON s.user_id = u.login
                JOIN "realtimeGraph_location" l ON s.location_id = l.id
                JOIN "realtimeGraph_city" ci ON l.city_id = ci.id
                JOIN "realtimeGraph_state" st ON l.state_id = st.id
                JOIN "realtimeGraph_country" co ON l.country_id = co.id
                JOIN "realtimeGraph_measurement" m ON d.measurement_id = m.id
                WHERE {where_sql}
            ),
            station_stats AS (
                SELECT station_id,
                       AVG(individual_value) AS global_avg,
                       STDDEV(individual_value) AS global_stddev,
                       COUNT(*) AS total_readings
                FROM expanded GROUP BY station_id
            )
            SELECT e.base_time, e.individual_value, e.individual_time_offset,
                   e.user_login, e.city_name, e.state_name, e.country_name,
                   e.measurement_name, e.measurement_unit,
                   ss.global_avg, ss.global_stddev, ss.total_readings,
                   time_bucket('1 hour', e.base_time) AS bucket,
                   ROUND(CAST(ABS(e.individual_value - ss.global_avg) / NULLIF(ss.global_stddev, 0) AS numeric), 2) AS deviation_factor
            FROM expanded e
            JOIN station_stats ss ON e.station_id = ss.station_id
            WHERE ss.global_stddev > 0
              AND ABS(e.individual_value - ss.global_avg) > %s * ss.global_stddev
            ORDER BY e.base_time DESC, e.individual_time_offset DESC
        """
        params.append(threshold)

        count_query = f"""
            SELECT COALESCE(SUM(d.length), 0)
            FROM "realtimeGraph_data" d
            JOIN "realtimeGraph_station" s ON d.station_id = s.id
            JOIN "realtimeGraph_location" l ON s.location_id = l.id
            JOIN "realtimeGraph_city" ci ON l.city_id = ci.id
        ...
        anomalies = []
        for r in rows:
            actual_time = r["base_time"].timestamp() + r["individual_time_offset"]
            anomalies.append({
                "time": datetime.fromtimestamp(actual_time, tz=timezone.utc).isoformat(),
                "bucket": r["bucket"].isoformat() if hasattr(r["bucket"], "isoformat") else str(r["bucket"]),
                "value": r["individual_value"],
                "measurement": r["measurement_name"],
                "unit": r["measurement_unit"],
                "user": r["user_login"],
                "location": f"{r['city_name']}, {r['state_name']}, {r['country_name']}",
                "station_avg": round(float(r["global_avg"]), 2),
                "station_stddev": round(float(r["global_stddev"]), 2),
                "deviation_factor": float(r["deviation_factor"]),
            })
        ...
        return JsonResponse({
            ...
            "summary": {
                ...
                "anomaly_percentage": round(
                    (anomaly_count / total_readings * 100) if total_readings > 0 else 0, 2
                ),
            },
        })
