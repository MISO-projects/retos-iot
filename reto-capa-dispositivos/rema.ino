...
void setup()
{
  ...
  #ifdef CHECK_CA_ROOT
    static BearSSL::X509List cert(digicert);
    net.setTrustAnchors(&cert);
  #endif
  #ifdef CHECK_PUB_KEY
    static BearSSL::PublicKey key(pubkey);
    net.setKnownKey(&key);
  #endif
  ...
  //Configurar tamaño de buffer para BearSSL (reduce uso de memoria)
...
//Función que valida si los valores del sensor son razonables
//DHT11: Humedad 20-90%, Temperatura 0-50°C
bool isValidReading(float h, float t) {
  if (isnan(h) || isnan(t)) return false;
  if (h < 0 || h > 100) return false;
  if (t < -10 || t > 60) return false;
  return true;
}

//Función que lee el sensor con reintentos
bool readDHTWithRetry(float &h, float &t, int maxRetries) {
  for (int i = 0; i < maxRetries; i++) {
    delay(100); //Pequeña pausa antes de leer
    h = dht.readHumidity();
    t = dht.readTemperature();
    if (isValidReading(h, t)) {
      return true;
    }
    Serial.println("Lectura inválida del sensor, reintentando...");
...
void loop()
{
  ...
  //Lee los datos del sensor con reintentos (hasta 3 intentos)
  float h, t;
  if (readDHTWithRetry(h, t, 3)) {
    ...
    Serial.println("Error: No se pudo obtener una lectura válida del sensor DHT11");
  }

  //Lee el valor del sensor LDR (luminosidad)
  int ldrRaw = analogRead(LDR_PIN);
  float lightPercent = (ldrRaw / 1023.0) * 100.0;
  
  //JSON para luminosidad
  String jsonLight = "{\"value\": "+ String(lightPercent, 1) + "}";
  char payload3[jsonLight.length()+1];
  jsonLight.toCharArray(payload3, jsonLight.length()+1);
  
  //Publica en el tópico de luminosidad
  client.publish(MQTT_PUB_TOPIC3, payload3, false);
  
  //Imprime en el monitor serial la información de luminosidad
